...
def cross_reference(mpc_candidates, browse_ids, tracker_desigs):
    """
    Find NEW candidates: on MPC NEOCP but NOT in NASA Browse catalog.
    These are genuine new discoveries not yet cataloged by NASA.
    """
    new_candidates = []
    for c in mpc_candidates:
        desig = c['desig']
        # Check if this designation appears in NASA's Browse catalog
        # MPC designations are temporary; NASA assigns permanent numbers
        # A candidate is "new" if it's NOT yet in our NASA DB
        # Since NASA DB uses neo_id (numeric), and MPC uses provisional desigs (alphanumeric),
        # we can't directly compare. Instead, we use the tracker DB as proxy:
        # If a candidate is on MPC but NOT in our tracker DB, it's newly appeared.
...
def generate_html(stats, approaches, mpc_candidates, new_candidates, tracker_total, last_update):
    ...
    # --- New discoveries (cross-reference) ---
    new_rows = ''
    if new_candidates:
        ...
        new_rows = '        <tr class="no-new"><td colspan="7">No new discoveries this cycle — all MPC candidates already in NASA catalog</td></tr>\n'
...
{mpc_rows}                    </tbody>
        ...
        <div class="card" style="grid-column: 1 / -1;">
            <div class="card-header">&#127775; New Discoveries — Cross-Reference (MPC &#8745; NASA Browse)</div>
...
{year_bars}            </div>
    ...
    <footer class="footer">
        ...
        Cross-reference: MPC candidates not yet in NASA Browse catalog = new discoveries
    </footer>
</body>
</html>'''
    return html
...
def main():
    ...
    print(f"  New candidates (on MPC but not in tracker): {len(new_candidates)}")
    if new_candidates:
